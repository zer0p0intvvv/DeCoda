...
class Cluster_GT(torch.nn.Module):
    ...
    def forward(self, data):
        # Handle AST_MALWARE dataset embedding
        if self.is_ast_dataset:
            # For AST_MALWARE, data.x contains [type_id, name_id, value_id] indices
            # Shape: [num_nodes, 3]
            type_ids = data.x[:, 0].long()
            name_ids = data.x[:, 1].long()
            value_ids = data.x[:, 2].long()
            
            # Get embeddings
            type_emb = self.type_embedding(type_ids)
            name_emb = self.name_embedding(name_ids)
            value_emb = self.value_embedding(value_ids)
            
            # Concatenate embeddings
            x = torch.cat([type_emb, name_emb, value_emb], dim=-1)
        else:
            # Original logic for other datasets
            if not self.use_rw:
                x = data.x
            else:
                if hasattr(data, 'rw_pos_enc') and data.rw_pos_enc is not None:
